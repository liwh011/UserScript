...
const script = {
    data() {
        return {
            ...
            // banWordList: GM_getValue('banWordList', config.屏蔽词列表),
            // showBanTip: GM_getValue('showBanTip', config.是否显示已被屏蔽条目),
            // setUninterested: GM_getValue('setUninterested', config.是否设置不感兴趣),
            // banUninterestedTag: GM_getValue('banUninterestedTag', config.是否设置不感兴趣TAG),
            config,
        }
    },
    methods: {
        ...
        onBanWordListChange() {
            // GM_setValue('banWordList', this.config.banWordList)
            // this.config.屏蔽词列表 = this.config.banWordList
        }
    },
    watch: {
        // showBanTip(v) { this.config.是否显示已被屏蔽条目 = v },
        // setUninterested(v) { this.config.是否设置不感兴趣 = v },
        // banUninterestedTag(v) { this.config.是否设置不感兴趣TAG = v },
    },
    mounted() {
